...
static napi_value fnGetProcessCommandLineByPid(napi_env env, napi_callback_info info)
{
    napi_status status;
    size_t argc = 1;
    napi_value argv[1];
    ...
    {
        napi_throw_type_error(env, NULL, __FUNCTION__);
        return NULL;
    }
    ...
    {
        napi_throw_type_error(env, NULL, string(__FUNCTION__).append("-> argv[0] mustBe napi_number").c_str());
        return NULL;
    }

    int processId = 0;
    napi_get_value_int32(env, argv[0], &processId);

    try
    {
        long status2 = 0;
        wstring CommandLine = GetProcessCommandLineByPid(processId, &status2);

        if (status2 == 0)
        {
            status = napi_create_string_utf16(env, (const char16_t *)CommandLine.c_str(), CommandLine.length(), &result);
            assert(status == napi_ok);
        }
    }
    HMCERR_Catch;
...
static napi_value fnGetCurrentWorkingDirectory(napi_env env, napi_callback_info info)
{
    ...
    try
    {
        wstring CommandLine = GetCurrentWorkingDirectory(processId);
...
static napi_value Init(napi_env env, napi_value exports)
{
    napi_property_descriptor BIND_NAPI_METHOD[] = {
        ...
        {"GetProcessCommandLineByPid", 0, fnGetProcessCommandLineByPid, 0, 0, 0, napi_default, 0},
        {"GetCurrentWorkingDirectory", 0, fnGetCurrentWorkingDirectory, 0, 0, 0, napi_default, 0}
