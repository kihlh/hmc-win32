...
namespace hmc_string_util
{
    ...
    void vec_to_array_json(PtrT &stringPtr, const std::vector<T> &data_list);
    /**
     * @brief 一个空的json 数组文本
     *
     * @return string
     */
    string vec_to_array_json();
    /**
     ...
     * @return string
     */
    wstring vec_to_array_json(vector<wstring> item_list);
    /**
     * @brief cpp内容数组转为json的array文本
     *
     * @tparam T
     ...
     */
    template <typename T>
    string vec_to_array_json(const std::vector<T> &data_list);
