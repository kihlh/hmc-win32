 ...
 */
const fn_type_list: fn_Analysis[] = [];
...
for (let index = 0; index < (data_list ? data_list?.length : 0); index++) {
    const data = data_list?.[index] || "";
    let function_header = data.match(/^(.+?) ([^ ]+)(?: +)?\(/i);
    if (!function_header) continue;
    ...
    const fn_Analysis: fn_Analysis = {
        name: fn_name,
        type: result_type as cpp_type,
        argc: 0,
        argv: [],
        exports: fn_name.replace(/fn_AU3_/, ""),
        js_type: cpp_type_to_js_type(result_type as cpp_type)
    };

    for (let index = 0; index < fn_args_list.length; index++) {
        const element = fn_args_list[index].replace(/^ +| +$/, "");
        if (!element || element.match(/^void$/)) continue;
        const data = {
            type: (element.match(/^(?: +)?(.+?) /)?.[1] || "any") as cpp_type,
