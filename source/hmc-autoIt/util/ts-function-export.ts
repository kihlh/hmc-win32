...
const fn_type_list: fn_Analysis[] = JSON.parse(fs.readFileSync("../AutoItX3.hpp.json", "utf-8"));


export type cpp_type = "int" | "wstring" | "string" | "bool" | "void" | "long long" | "HWND" | "RECT" | "POINT" | "unsigned int" | "any" | "LPRECT" | "int64_t";
export type js_type = "undefined" | "string" | "boolean" | "number" | "bigint" | "null" | "{x:number, y:number}" | "{left:number, top:number,right:number, bottom:number}";

interface fn_Analysis {
    name: string,
    type: cpp_type,
    argc: number,
    argv: {
    ...
    description?: {
        "name": string,
        "description": { name: string, defaultv: null, type: js_type, desc: string }[],
        "returns": "",
        [key: string]: string | { name: string, defaultv: null, type: js_type, desc: string }[],
    }
}

function cpp_type_to_js_type(type: cpp_type): js_type {
...
function get_description(fn_name: string) {
    let description = "\n// " + fn_name;
...
for (let index = 0; index < fn_type_list.length; index++) {
    ...
    for (let index = 0; index < fn_type.argv.length; index++) {
        ...
        if (!argv.defaultv) {
            ...
            args_fn = args_fn + `${argv.name} : ${cpp_type_to_js_type(argv.type)} = ${argv.defaultv?.replace(/L"/, "\"")}, `;
        }
        ...
        switch (cpp_type_to_js_type(argv.type)) {
            ...
            case "undefined":
                ...
                break;
            case "string":
                args_eval = args_eval + ` ref.string(typeof ${argv.name} == "undefined" ? ${argv.defaultv ? argv.defaultv?.replace(/L"/, "\"") : "\"\""} : ${argv.name} ) ,\n`;
                break;
            case "bigint":
